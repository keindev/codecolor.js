...
const scss: ISchema = {
  name: 'scss',
  expressions: {
    names: ['comment', 'interpolation', 'string', 'keyword', 'entity', 'variable', 'operator', 'constant', 'source'],
    values: [
      ['\\/\\*[\\s\\S]*?\\*\\/|\\/\\/.*$'],
      ['(["\'])(?:(?=(\\\\?))\\2.)*?\\1'],
      ['(?<=url\\()[:;,=\\w\\/.\\d\\s]+(?=\\))'],
      ['[\\[\\]]|(!(important|default))', '@\\w*(?=\\s*{*)'],
      ['#\\w+(?=[ \\t\\.{\\[\\(])', '(?<=#)([A-Fa-f\\d]{6}|[A-Fa-f\\d]{3})'],
      [
        '#{.+[^{]}',
        '(?<=:\\s*var)\\([\\w\\-]*\\)(?=\\s*;)',
        '(?<=[{;]\\s*)--[\\w\\-]*(?=\\s*:)',
    ...
    values: [
      {
        ...
        b: ['b', 'base', 'big', 'bdi', 'bdo', 'blockquote', 'body', 'br', 'button'],
        ...
        h: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hgroup', 'head', 'header', 'hr', 'html'],
        i: ['i', 'iframe', 'img', 'input', 'ins'],
        k: ['kbd', 'keygen'],
        l: ['label', 'legend', 'li', 'link'],
        m: ['main', 'map', 'mark', 'meta', 'meter'],
